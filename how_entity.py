#!/usr/bin/env python3
"""Explain how an entity was resolved."""
...
async def how_entity(entity_id):
    """Explain how an entity was resolved."""
    ...
    try:
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                result = await session.call_tool("explain_entity_resolution", {
                    "entity_id": int(entity_id)
                })

                how_data = json.loads(result.content[0].text)

                if "error" in how_data:
                    print(f"❌ {how_data['error']}")
                else:
                    print("✅ Entity Resolution Explanation:\n")
                    print(json.dumps(how_data, indent=2))
...
if __name__ == "__main__":
    ...
    asyncio.run(how_entity(entity_id))
